...
def parse_ldblk(ldblk_dir, sst_dict, chrom):
    ...
    for blk in range(n_blk):
        ...
        if idx != []:
            ...
            print("max:", np.max(ld_blk[blk]))
            print("mean:", np.mean(ld_blk[blk]))
...
def parse_ldblk_from_dict(ldgm_blocks, sst_dict, chrom):
    print('... parse reference LD on chromosome %d ...' % chrom)

    ld_blk = []
    snp_blk = []

    sorted_blk_keys = sorted(ldgm_blocks.keys(), key=lambda x: int(x.split('_')[1]))

    for blk_key in sorted_blk_keys:
        blk_data = ldgm_blocks[blk_key]

        if blk_data["CHROM"] != chrom:
            continue

        snps = blk_data["snplist"]
        ld_blk.append(blk_data["ldblk"])
        snp_blk.append(snps)

    blk_size = []
    mm = 0

    for blk in range(len(snp_blk)):
        idx = [ii for (ii, snp) in enumerate(snp_blk[blk]) if snp in sst_dict['SNP']]
        blk_size.append(len(idx))
        ...
        if idx:
            idx_blk = range(mm, mm + len(idx))
            flip = [sst_dict['FLP'][jj] for jj in idx_blk]
            sub_ld = ld_blk[blk][np.ix_(idx, idx)] * np.outer(flip, flip)

            # SVD 修正对称性
            # _, s, v = linalg.svd(sub_ld)
            # h = np.dot(v.T, np.dot(np.diag(s), v))
            # ld_blk[blk] = (sub_ld + h) / 2

            _, s, v = linalg.svd(sub_ld)
            # 将所有小于0的奇异值设置为一个很小的正数 1e-6，目的是避免构造出来的矩阵中含有负的特征值，进而不是正定的。
            s[s < 0] = 1e-6
            h = np.dot(v.T, np.dot(np.diag(s), v))
            sub_ld_sym = (sub_ld + h) / 2
            scale = np.mean(np.diag(sub_ld_sym))  # 计算对角线元素的均值
            epsilon = 1e-6 * scale  # 设置 epsilon 为对角元素均值的比例

            # 最后加上 epsilon，确保正定性
            ld_blk[blk] = sub_ld_sym + np.eye(sub_ld_sym.shape[0]) * epsilon
